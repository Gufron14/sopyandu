                                    ...
                                    <tbody>
                                        @foreach ($officers as $officer)
                                            <tr>
                                                ...
                                                <td class="text-center">{{ $officer->gender ?? 'N/A' }}</td>
                                                @if (auth()->check() && auth()->user()->role == 'admin')
                                                    <td class="font-italic">
                                                        {{ $officer->users->first()->username ?? 'N/A' }}
                                                    </td>
                                                @endif
                                                <td class="text-right">
                                                    {{ $officer->users->first()->phone_number ?? 'N/A' }}
                                                </td>
                                                <td>{{ $officer->position ?? 'N/A' }}</td>
                                                <td class="text-center">
                                                    @if (!empty($officer->users->first()->verified_at) || $officer->users->first()->verified_at !== null)
                                                ...
                                                <td>
                                                    <div class="d-flex justify-content-center" style="gap: .5rem">
                                                        <a href="{{ url("/officer-data/{$officer->id}/show") }}"
                                                            class="btn btn-info" data-toggle="tooltip" title="Detail">
                                                            <i class="fas fa-info-circle"></i>
                                                        </a>
                                                        @if (auth()->check() && auth()->user()->role == 'admin')
                                                            <a href="{{ url("/officer-data/{$officer->id}/edit") }}"
                                                                class="btn btn-primary" data-toggle="tooltip"
                                                                title="Ubah">
                                                                <i class="fas fa-pencil"></i>
                                                            </a>
                                                            @if ($officer->position !== 'Lurah')
                                                                <form action="{{ url("/officer-data/{$officer->id}") }}"
                                                                    method="POST" id="delete-form-{{ $officer->id }}"
                                                                    class="d-inline">
                                                                    @method('delete')
                                                                    @csrf
                                                            ...
                                                            @endif
                                                            <form action="{{ url("/officer-data/{$officer->id}/reset") }}"
                                                                method="POST" id="reset-form-{{ $officer->id }}"
                                                                class="d-inline">
                                                                @method('PUT')
                                                                @csrf

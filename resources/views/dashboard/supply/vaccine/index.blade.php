                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                @php
                                    // Mengelompokkan vaksin berdasarkan nama dan jenis
                                    $groupedVaccines = $vaccines->groupBy('vaccine_name');
                                @endphp
                                ...
                                <table class="table-striped table" id="table-1">
                                    ...
                                    <tbody>
                                        @php $no = 1; @endphp
                                        @foreach ($groupedVaccines as $vaccineName => $vaccineGroup)
                                            @foreach ($vaccineGroup as $index => $vaccine)
                                                <tr>
                                                    <td class="text-right">{{ $no++ }}</td>

                                                    {{-- Tampilkan hanya pada baris pertama per kelompok --}}
                                                    @if ($index === 0)
                                                        <td class="text-center" rowspan="{{ $vaccineGroup->count() }}">
                                                            {{ $vaccine->vaccine_type ?? 'N/A' }}
                                                        </td>
                                                    @endif

                                                    <td>{{ $vaccine->vaccine_name ?? 'N/A' }}</td>
                                                    <td>{{ $vaccine->unit ?? 'N/A' }}</td>
                                                    <td class="text-center">{{ $vaccine->stock ?? 'N/A' }}</td>
